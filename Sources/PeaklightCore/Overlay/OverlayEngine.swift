...
enum OverlayWindowPolicy {
  // Covers status icons and the clock while popup menus remain above the
  // attenuation layer, preserving a visible recovery path.
  static let whiteDimmingLevel = NSWindow.Level(
    rawValue: NSWindow.Level.statusBar.rawValue + 1
  )

  // Mission Control hides transient windows. A full-resolution backdrop
  // layer must not remain over the scaled overview thumbnails.
  static let whiteDimmingCollectionBehavior: NSWindow.CollectionBehavior = [
    .canJoinAllSpaces,
    .transient,
    .ignoresCycle,
    .fullScreenAuxiliary,
